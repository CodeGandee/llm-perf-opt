...
- [ ] T007 Implement profiling data models in `src/llm_perf_opt/profiling/models.py` (ProfilingSession, ModelTarget, StageTiming, OperatorRecord, KernelRecord via `attrs`)
...
- [ ] T011 [P] Add contract→CLI mapping note in `specs/002-nvidia-llm-profiling/contracts/MAPPING.md` (map `/profile/run` → Pixi `stage2-profile`, `/profile/{run_id}/artifacts` → list `tmp/stage2/<run_id>`)
