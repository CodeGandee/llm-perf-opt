...
- Partially done / still TODO:
  ...
  - `LlamaFlashAttention2.forward_cal_io()` and `forward_memory_activation()` have been updated to use query length (`m_seq_len_q`) for I/O and activation modeling; decode mode is now query/context-aware while keeping the legacy `seq_len` field for backward compatibility in documentation and verification helpers.
