...
本报告覆盖解码阶段观察到的 Top 20 内核的完整剖析数据。
...
对 Top 20 内核全部进行了 NCU 剖析，成功率 100%。基于屋脊线分析进行分类：
...
**分类定义：**
1. **计算受限**：算术强度 ≥ 分界点，且 SM/张量核利用显著高于内存吞吐；瓶颈在计算吞吐、指令级并行（ILP）/铺片或调度。
2. **内存受限**：算术强度 < 分界点，且内存吞吐显著高于 SM 吞吐；瓶颈在 DRAM/L2 带宽或延迟以及数据局部性（访存合并/复用）。
3. **均衡**：计算与内存利用均未接近各自屋脊（双低）；通常可通过融合、专用化与数据布局优化获得收益。

**阈值设置：**
1. **分界点**：本工作负载取 80 FLOPs/byte（由 P_peak / BW_eff 估算；观测范围约 50–100 FLOPs/byte，分类时固定为 80）。
2. **利用率优势**：“显著高于”定义为比值 ≥ 1.3× 或绝对差 ≥ 5 个百分点（pp），比较对象为 SM 吞吐与内存吞吐。
3. **低利用率门槛**：若 SM 与内存吞吐均 < 20% 峰值，则归为“均衡”，不受算术强度影响。
...
| L1 命中率 | 6.54% | 9.51% | 9.65% | 8.96% |
| L2 命中率 | 71.93% | 16.40% | 51.97% | 39.95% |
| 平均时长 | 137.44 μs | 9.67 μs | 39.75 μs | 45.75 μs |

要点：
1. **完整数据集**：20 个内核全部完成分类。
2. **资源利用率**：总体均值显示利用率仍偏低（SM 15.75%、内存 34.62%、实际占用率 39.51%）。
3. **内存受限占比（45%）**：近半数内核受限于内存带宽。
...
**表格解读提示：**
1. **按内核平均**：该表为“按内核平均”的分类均值（未按运行总时长加权）；“总体均值”为 20 个内核的简单平均。
2. **计算受限特征**：SM 吞吐更高（36.21%）、平均更长（137.44 μs），应优先优化张量核利用、铺片与指令级并行。
3. **内存受限特征**：DRAM/内存吞吐更高（34.23%/41.20%）、平均更短（9.67 μs），应优先做融合、布局与复用优化。
4. **均衡特征**：计算与内存均未接近屋脊，适合考虑融合或专用化。
5. **缓存命中注意**：命中率受访问路径（如流式加载、cache‑bypass、cp.async→共享内存）影响较大，仅作参考；每内核细节请见原始 NCU 数据。
...
分析：
- 最长内核为 404.70 μs：表明受大分块 GEMM 影响存在长尾。
- 记忆高效注意力（160.74 μs）：归为“均衡”，为第 2 慢内核，说明注意力计算贡献显著。
...
1. 覆盖完整：Top 20 全部完成剖析（100% 覆盖）。
2. GEMV 瓶颈（49.2% 时间）：自回归解码以 GEMV 为主（batch=1），天然更偏内存受限，计算利用率在现代 GPU 上不高。
3. 低利用率（绝对值）：SM 吞吐 15.75%、内存吞吐 34.62%、实际占用率 39.51%，仍有较大优化空间。
4. 内存层级：L1 命中率 8.96%，L2 命中率 39.95%，kernel_0009（记忆高效注意力）与 kernel_0016（CUTLASS GEMM）缓存局部性较差。
5. 内核融合机会：大量小型点算子（乘、加、激活等）共占 21.7% 时间，具备显著融合潜力，可降低访存与启动开销。
6. 混合精度开销：频繁的 BF16 ↔ FP32 转换（拷贝/类型转换约 6%）显示数据类型管理可进一步优化。
7. 注意力内核洞见：kernel_0009（记忆高效注意力）单次 160.74 μs，归为“均衡”；与 FlashAttention（10.34 μs）相比差距 15.5×，存在替换/优化空间。
8. 计算受限占比：4 个（20%），该类内核平均 137.44 μs，显著长于内存受限（9.67 μs）与均衡（39.75 μs）。
...
发现：L1 命中率 8.96%，L2 命中率 39.95%，表明工作集超出 L1，且仅部分可在 L2 得到复用。
...
- 分界点（ridge point）：P_peak / BW_eff；本次分类取 80 FLOPs/byte（观测范围约 50–100）。
- 分类规则与阈值：
  - 计算受限：AI ≥ 分界点，且（SM 吞吐 ≥ 1.3× 内存吞吐，或 SM − 内存 ≥ 5 个百分点）；
  - 内存受限：AI < 分界点，且（内存吞吐 ≥ 1.3× SM 吞吐，或 内存 − SM ≥ 5 个百分点）；
  - 均衡：其余情况；另外若 SM 与内存吞吐均 < 20%，直接判为“均衡”。

**分界点取值依据（为何选 80 FLOPs/byte）：**
1. 公式：分界点 = 计算峰值 P_peak / 有效带宽 BW_eff（FLOP/s ÷ B/s），对应屋脊线中内存/计算受限的交界。
2. 计算屋脊选择：解码阶段混合了大量 elementwise/ATen 与 GEMV/GEMM，许多内核不能充分使用张量核，因此以 FP32 SM 峰值作为更稳健的“计算屋脊”，而非更高的张量核峰值。
3. 观测区间：归一化/物理屋脊线呈现约 50–100 FLOPs/byte 的过渡带；固定 80 作为中位值，保证跨多次运行与不同内核类型的分类稳定性。
4. 数值示例（量级）：FP32 峰值 ≈ 105 TFLOP/s；本解码工作负载的有效带宽 ≈ 1.3 TB/s ⇒ 105 / 1.3 ≈ 80 FLOPs/byte（示意；BW_eff 随运行有波动）。
5. 可选策略：按运行重算分界点 P_peak(dType)/BW_eff（P_peak 选取契合主导计算模式，BW_eff 可取内存受限内核的峰值或 95 分位）；若追求更高保真度，可按内核使用分界点，但本报告固定 80 以简化并提高一致性。
