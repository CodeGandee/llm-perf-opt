...
**阈值设置：**
1. **分界点**：本工作负载取 80 FLOPs/byte；分界点 = P_peak / BW_eff（计算峰值 ÷ 有效带宽）。观测到的过渡带约为 50–100 FLOPs/byte，我们固定 80 以保证稳定性。
2. **计算屋脊选择**：由于解码阶段大量 elementwise/ATen 与 GEMV 不能充分使用张量核，采用 FP32 SM 峰值作为“计算屋脊”。
3. **数值示例**：FP32 峰值 ≈ 105 TFLOP/s；有效带宽 ≈ 1.3 TB/s ⇒ 105 / 1.3 ≈ 80 FLOPs/byte（示意，BW_eff 随运行波动）。
4. **利用率优势**：“显著高于”定义为比值 ≥ 1.3× 或绝对差 ≥ 5 个百分点（pp），比较对象为 SM 吞吐与内存吞吐。
5. **低利用率门槛**：若 SM 与内存吞吐均 < 20% 峰值，则归为“均衡”，不受算术强度影响。
6. **可选策略**：按运行重算分界点 P_peak(数据类型)/BW_eff（GEMM 偏重可用 BF16 张量峰值）；BW_eff 可取内存受限内核峰值或 95 分位。也可按内核设置分界点，但本报告固定 80 以简化并增强一致性。
