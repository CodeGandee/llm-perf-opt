...
For capacity planning it is often useful to have a single VRAM envelope per crop grid.
Using the same assumptions as in the perf report (TTFT = 1.0 s for vision+prefill and TPOT = 0.05 s with `K = 100` decode steps), we approximate VRAM as:
...
Concretely, for each crop grid:

`VRAM needed (GB) ≈ 6.34 (model weights) + Peak activation size (GB) + KV-cache size (GB)`.

Because the current analytic `weights_gb` under-estimates MoE expert parameters, the weights term in this table is overridden by the static torchinfo value.

The table below reuses the VRAM figures from the perf report, summarizing, for each crop configuration, the combined vision+prefill FLOPs, activation I/O, KV-cache size, peak activations, and the resulting approximate VRAM requirement.
